            ...
            onClick={() => {
              onAction(member.id, "toggle", () =>
                member.status === "Active"
                  ? disableEmployee(member.id)
                  : enableEmployee(member.id),
              );
              setPos(null);
            }}
            disabled={isLoading}
              ...
              <button
                ...
                onClick={() => {
                  onAction(member.id, role, () =>
                    hasRole
                      ? revokeRole(member.id, role)
                      : grantRole(member.id, role),
                  );
                  setPos(null);
                }}
                disabled={isLoading}
...
export default function OrganizationMembersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(
        "Dodajanje zaposlenega ni uspelo. Preverite podatke in poskusite znova.",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        {loadError ? (
          ...
          <div className="flex flex-col items-center px-6 py-14 text-center">
            ...
            <h3 className="font-semibold text-slate-800">
              Organizacija še nima zaposlenih
            </h3>
            <p className="mt-1 text-sm text-slate-500">
              Dodajte prvega zaposlenega z gumbom zgoraj.
            </p>
          </div>
        ) : (
          <>
